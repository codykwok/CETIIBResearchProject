...
runnumber = input('Run number: ','s'); % Input run number between 1-9
...
ovenheatloss = heatloss(ssovenloc,kback);
discrepancy(str2double(runnumber)) = ovenheatloss(str2double(runnumber)) - sspower(str2double(runnumber));
discrepancypercentage = round(100*(1 - ovenheatloss/sspower),2);

% Function to calculate rate of heat loss of oven
function heat = heatloss(temps,kback)
...
d6out = 3.86*10^-3;
L = 0.325;
kair = (0.00031417*(273.15+temps(7))^0.7786)/(1-0.7116/(273.15+temps(7)) + 2121.7/(273.15+temps(7))^2);
g = 9.81;
rhoair = 28.97*101325/(8.3145*10^3*(273.15+temps(7)));
beta = 1/(273.15+temps(7));
Pr = 0.71;
nu = 1.46*10^-5;
DeltaT = mean(temps([7,13])) - mean(temps([6,7]));
kglass = 1.4;
kwool = 0.1163;
ksteel = 13.4;
dsteel = 0.005;
