...
for runnumber = 11:11 % Input run number between 11-19, can input multiple or individual runs
    ...
    batstart = [4548,4323.23,4530.43,4538.43,4526.81,4557.27,4539.69,4545.72,4522.63];
    batend = [12020.8,11448.4,11676.1,11890.8,12068.2,12028.6,11886,11966.4,11923.8];
    ...
    cuwater = [2.26,2.217,2.158,2.189,2.208,2.374,2.275,2.149,2.31];
    ...
    for i = 1:31
        ...
        batstartindex = find(holdovenloci(:,1) > batstart(str2double(runnumber)-30),1,'first');
        batendindex = find(holdovenloci(:,1) > batend(str2double(runnumber)-30),1,'first') - 1;
    ...
    batstartindex = find(roomtemp(:,1) > batstart(str2double(runnumber)-30),1,'first');
    batendindex = find(roomtemp(:,1) > batend(str2double(runnumber)-30),1,'first') - 1;
    batovenloc(:,32) = mean(roomtemp(batstartindex:batendindex,2));
    batstartindexp = find(energy(:,1) > batstart(str2double(runnumber)-30),1,'first');
    batendindexp = find(energy(:,1) > batend(str2double(runnumber)-30),1,'first') - 1;
    batpower = (energy(batendindexp,2)-energy(batstartindexp,2))*3600000./(batend(str2double(runnumber)-30)-batstart(str2double(runnumber)-30));
    ...
    cuwaterenergy = heatsinkwater(cutovenloc,cuwater(runnumber-30));
    ...
    totalenergy = airoutenergy+cuwaterenergy+cuumaterialenergy+heatlossovenouts+heatlosscuouts+tubingloss;
    discrepancy = (totalenergy-batenergy)/batenergy*100;
    extraefficiency = cuwaterenergy / totalenergy;
    table(runnumber,batend(runnumber-30)-batstart(runnumber-30),batenergy,totalenergy,round(discrepancy,2),heatlossovenouts,tubingloss,heatlosscuouts+cuumaterialenergy,airoutenergy,cuwaterenergy,'VariableNames',{'Runnumber','Elapsed Time (s)','Measured Energy (J)','Calculated Energy (J)','Energy Discrepancy (%)','Oven Loss (J)','Tubing Loss (J)','Cooler Unit Loss (J)','Air Out Loss (J)','Cooler Unit Recovered Energy (J)'})
end
